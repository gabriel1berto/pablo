  ...
  return (
    <main style={{ minHeight: "100vh", display: "flex", flexDirection: "column", maxWidth: 480, margin: "0 auto", padding: "0 24px" }}>
      ...
      {status === "reset-sent" ? (
        ...
        <>
          {/* Google */}
          <form onSubmit={async (e) => { e.preventDefault(); setStatus("loading"); const r = await signInWithGoogle(); if (r.url) { window.location.href = r.url; } else { setErro("Erro ao conectar com Google."); setStatus("error"); } }} style={{ marginTop: 24 }}>
            <button
              type="submit"
